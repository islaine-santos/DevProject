...
export function PerfilProfissional() {
  ...
  useEffect(() => {
    async function fetch() {
      ...
      const { data: prof } = await supabase
        .from('professionals')
        .select('*, user:users!user_id(id, nome, email, avatar_url, created_at), services:professional_services(service:services(*))')
        .eq('user_id', id)
        .single();
  ...
  const profileServices = ((profile as unknown as Record<string, unknown>).services as Array<{ service: { nome: string } }> | undefined) ?? [];

  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <Link to="/profissionais" className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-primary-600 mb-6">
        ...
        <div className="flex flex-col sm:flex-row items-start gap-6">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-1">
              ...
              <SeloBadge selo={profile.selo} />
              {profile.plano !== 'free' && (
                <span className={classNames('inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium', PLAN_COLORS[profile.plano])}>
                  <CheckCircle className="w-3 h-3" /> {PLAN_LABELS[profile.plano]}
                </span>
              )}
            </div>

            <div className="flex items-center gap-4 text-sm text-gray-500 mb-3">
              <span className="flex items-center gap-1"><MapPin className="w-4 h-4" /> {profile.cidade}, {profile.estado}</span>
            ...
            <div className="flex flex-wrap gap-2">
              {profileServices.map((ps) => (
                <span key={ps.service.nome} className="bg-primary-50 text-primary-700 px-3 py-1 rounded-full text-xs font-medium">
                  {ps.service.nome}
                </span>
              ))}
            </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 md:p-8">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">Avaliações ({reviews.length})</h2>
        {reviews.length === 0 ? (
          <p className="text-gray-400 text-sm">Nenhuma avaliação ainda.</p>
        ) : (
          <div className="space-y-4">
            {reviews.map((review) => {
              const reviewerName = review.avaliador && typeof review.avaliador === 'object' && 'nome' in review.avaliador
                ? (review.avaliador as { nome: string }).nome : 'Anônimo';
              return (
                <div key={review.id} className="border-b border-gray-100 pb-4 last:border-0 last:pb-0">
                  ...
                  {review.comentario && <p className="text-sm text-gray-600 mt-2">{review.comentario}</p>}
                </div>
              );
            })}
          </div>
